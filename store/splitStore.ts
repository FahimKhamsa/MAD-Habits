  ...
  persist(
    (set, get) => ({
      ...
      addGroup: async (name, members) => {
        ...
        try {
          await createGroupRemote(
            newGroup.id,
            name,
            membersWithIds.map((m) => ({
              name: m.name,
              email: m.email || '',
          ...
          console.error('Failed to sync group to database:', error)
          // Group is still in local state, which is fine for offline functionality
        }
